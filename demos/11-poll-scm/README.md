...
- Difference between **poll** (~1 minute lag) and **webhook** (near-instant)

## Instructor talking points

1. **Build Now once first** — the Jenkinsfile must be loaded before `pollSCM` is registered on the job.
2. **Push to main → wait up to ~60s** — schedule `* * * * *` means about once a minute; look for cause **Started by an SCM change**.
...
2. Confirm **Configure** → **Build Triggers** shows Poll SCM with `* * * * *`.
3. Make a small change on `main` (for example edit a comment in `app.py`) and **push** to GitHub.
4. Stay on the Jenkins job page — within about **one minute** a new build should appear without clicking Build Now.
